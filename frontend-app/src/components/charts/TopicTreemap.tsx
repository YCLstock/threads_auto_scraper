...
export default function TopicTreemap({ data }: TopicTreemapProps) {
  ...
  useEffect(() => {
    ...
    const colorScale = d3.scaleOrdinal()
      .domain(data.map(d => d.topic_name))
    ...
    const rects = cells.append('rect')
      .attr('width', d => Math.max(0, d.x1 - d.x0))
      .attr('height', d => Math.max(0, d.y1 - d.y0))
      .attr('fill', d => `url(#gradient-${d.data.data.topic_id})`)
      .attr('stroke', '#fff')
      .attr('stroke-width', 2)
      .style('opacity', 0)
    ...
    const statsLabels = cells.append('text')
      ...
      .style('text-shadow', '1px 1px 2px rgba(0,0,0,0.7)')
      .style('opacity', 0)
      .text(d => `${d.data.data.post_count} 篇`)
    ...
    const interactionLabels = cells.append('text')
      ...
      .style('text-shadow', '1px 1px 2px rgba(0,0,0,0.7)')
      .style('opacity', 0)
      .text(d => `${d.data.data.total_interactions.toLocaleString()} 互动`)
    ...
    const sentimentIndicators = cells.append('circle')
      ...
      .attr('cy', 15)
      .attr('r', 6)
      .attr('fill', d => sentimentColors[d.data.data.dominant_sentiment as keyof typeof sentimentColors] || sentimentColors.neutral)
      .attr('stroke', '#fff')
      .attr('stroke-width', 2)
      .style('opacity', 0)
    ...
    const trendIndicators = cells.append('path')
      .attr('d', d => {
        ...
        const y = 15
        const size = 5
        const trend = d.data.data.trending_score
        if (trend > 0.7) {
          // 向上箭头
          return `M${x},${y+size} L${x+size},${y-size} L${x-size},${y-size} Z`
        } else if (trend < 0.3) {
